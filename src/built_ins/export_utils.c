...
/*   Updated: 2024/06/20 19:51:06 by mokutucu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *find_variable(const char *arg)
{
	...
	return ft_shell_strndup(arg, var_name_len);
}
...
char **add_env_var(char *arg, char **env, int env_len)
{
	...
	new_env[env_len] = ft_shell_strdup(arg);
...
char **change_or_add_env_var(char *arg, char **env)
{
	...
	char *var_name = find_variable(arg);
	int var_idx = find_var_in_env(env, var_name);

	if (var_idx != -1)
	{
		env[var_idx] = ft_shell_strdup(arg);
	}
	else
	{
		env = add_env_var(arg, env, env_len);
	}
	return env;
}
